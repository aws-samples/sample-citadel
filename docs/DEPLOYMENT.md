...
# Alarm delivery (required for staging/prod; optional for dev/test)
ALARM_DELIVERY=email              # one of: email | slack | none
ALARM_EMAIL=oncall@example.com    # required when ALARM_DELIVERY=email
# ALARM_SLACK_WORKSPACE_ID=T0XXXXXXX  # required when ALARM_DELIVERY=slack
# ALARM_SLACK_CHANNEL_ID=C0XXXXXXX    # required when ALARM_DELIVERY=slack

# Trace backend for the waterfall trace viewer (optional; default: xray)
# TRACE_BACKEND=xray                # one of: xray | spans
```
...
**Note:** `ALARM_DELIVERY` selects who is subscribed to the platform SNS alarm topics — without it the CloudWatch alarms fire into topics with no subscriber. Leaving it unset is treated as `none` for dev/test, but is a **hard synth failure for staging/prod** (set it explicitly there, even to `none`); an invalid value always fails synth. `email` mode requires `ALARM_EMAIL` and the recipient must click the SNS confirmation link once before delivery works; `slack` mode requires both Slack IDs plus a one-time AWS Chatbot workspace OAuth authorization in the console that CDK cannot perform. See the alarm-delivery block in `backend/.env.example` for the full explanation, and [docs/OBSERVABILITY.md](OBSERVABILITY.md) for how to verify delivery.

**Note:** `TRACE_BACKEND` is a supported adopter choice for how the waterfall trace viewer reads trace data — `xray` (the default) reads from AWS X-Ray directly and works out of the box with no prerequisites; `spans` reads spans from CloudWatch Logs via Transaction Search. Selecting `spans` requires CloudWatch Transaction Search to be enabled in the target account/region first — without it, the viewer returns **empty results, not an error** (a silent-blindness failure mode). See [docs/TRACING_RUNBOOK.md](TRACING_RUNBOOK.md) for the cutover procedure before switching an environment from `xray` to `spans`.
