...
describe("classifyDrift", () => {
  ...
  // workflowIds drift (finding 4d69104a): bindWorkflowToApp/
  // unbindWorkflowFromApp now write workflowIds to both the registry
  // manifest and the #META mirror synchronously — a failed mirror write
  // must be caught as 'stale' rather than silently tolerated.
  it("returns 'stale' when workflowIds differs", () => {
    expect(
      classifyDrift(
        makeProjection({ workflowIds: ["wf-1", "wf-2"] }),
        makeMetaRow({ workflowIds: ["wf-1"] }),
      ),
    ).toBe("stale");
  });

  it("treats workflowIds as equal regardless of element order", () => {
    expect(
      classifyDrift(
        makeProjection({ workflowIds: ["wf-1", "wf-2"] }),
        makeMetaRow({ workflowIds: ["wf-2", "wf-1"] }),
      ),
    ).toBe("in-sync");
  });

  it("returns 'stale' when the mirror has an empty workflowIds array but the registry has entries", () => {
    expect(
      classifyDrift(
        makeProjection({ workflowIds: ["wf-1"] }),
        makeMetaRow({ workflowIds: [] }),
      ),
    ).toBe("stale");
  });

  it("returns 'in-sync' when fields differ on a key NOT in the comparison set", () => {
    // description, routingConfig, createdBy, createdAt, updatedAt all live
    // OUTSIDE the (name, status, orgId, version, workflowIds) drift
