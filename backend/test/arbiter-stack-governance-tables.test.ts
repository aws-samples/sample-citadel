...
// PythonFunction entry paths resolve from backend/lib/ via ../../../arbiter/*
// which lives outside the backend workspace. Stub the resolved paths so CDK
// can compute the asset hash without requiring a full repo checkout.
...
describe('ArbiterStack — US-ARB-002 governance tables (Δ8)', () => {
  ...
  describe('table presence', () => {
    ...
    test.each(expectedNames)('table %s exists with PAY_PER_REQUEST billing', (name) => {
  ...
  describe('DeletionPolicy', () => {
    ...
    test.each(retainedNames)('%s has DeletionPolicy=Retain and UpdateReplacePolicy=Retain', (name) => {
  ...
  describe('DeletionProtection', () => {
    ...
    test.each(protectedNames)('%s has DeletionProtectionEnabled=true', (name) => {
      template.hasResourceProperties('AWS::DynamoDB::Table', {
        TableName: name,
