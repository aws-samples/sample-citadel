...
describe("BackendStack — Workflow/App/Execution Lambda functions and AppSync wiring (Task 1.5)", () => {
  ...
  describe("AppSync Resolvers — Workflow", () => {
    ...
    test.each(workflowQueryFields)(
      "Query resolver for %s exists",
      (fieldName) => {
    ...
    test.each(workflowMutationFields)(
  ...
  describe("AppSync Resolvers — Execution", () => {
    ...
    test.each(executionQueryFields)(
      "Query resolver for %s exists",
      (fieldName) => {
        template.hasResourceProperties("AWS::AppSync::Resolver", {
          TypeName: "Query",
    ...
    test.each(executionMutationFields)(
      "Mutation resolver for %s exists",
      (fieldName) => {
        template.hasResourceProperties("AWS::AppSync::Resolver", {
          TypeName: "Mutation",
  ...
  describe("IAM Policies — least-privilege", () => {
    ...
    test("WorkflowResolver has DynamoDB read/write on Apps table (needed for importBlueprint)", () => {
      ...
      const policies = template.findResources("AWS::IAM::Policy");
      const wfResolverPolicies = Object.entries(policies).filter(
        ([, p]: [string, any]) => {
          const roles = p.Properties?.Roles || [];
      ...
      const hasAppsWritePolicy = wfResolverPolicies.some(
        ([, p]: [string, any]) => {
          const statements = p.Properties?.PolicyDocument?.Statement || [];
          return statements.some((s: any) => {
            const actions = Array.isArray(s.Action) ? s.Action : [s.Action];
            ...
            return resources.some((r: any) => {
              ...
              if (Array.isArray(getAtt) && getAtt[0]?.includes("AppsTable"))
                return true;
              const join = r?.["Fn::Join"];
              if (Array.isArray(join?.[1])) {
                return join[1].some((part: any) => {
                  if (typeof part === "string" && part.includes("AppsTable"))
                    return true;
                  const innerGetAtt = part?.["Fn::GetAtt"];
                  return (
                    Array.isArray(innerGetAtt) &&
                    innerGetAtt[0]?.includes("AppsTable")
                  );
                });
              }
              return false;
