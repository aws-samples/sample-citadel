...
export const handler: AppSyncResolverHandler<any, any> = async (event) => {
  ...
  try {
    switch (fieldName) {
      case 'getWorkflow':
        return await getWorkflow(args.workflowId, userId);
      case 'listWorkflows':
        return await listWorkflows(args.orgId, args.status, userId);
      case 'listBlueprints':
        return await listBlueprints(args.category);
      case 'createWorkflow':
        return await createWorkflow(args.input, userId);
      case 'updateWorkflow':
        return await updateWorkflow(args.input, userId);
      case 'deleteWorkflow':
        return await deleteWorkflow(args.workflowId, userId);
      case 'publishWorkflow':
        return await publishWorkflow(args.workflowId, userId);
      case 'updateWorkflowConfiguration':
        return await updateWorkflowConfiguration(args.workflowId, args.configuration, args.version, userId);
      case 'importBlueprint':
        return await importBlueprint(args.blueprintId, args.appId, args.name, userId);
      case 'importWorkflow':
        return await importWorkflowFn(args.input, userId);
      case 'exportWorkflow':
        return await exportWorkflow(args.workflowId, userId);
      case 'getWorkflowVersion':
        return await getWorkflowVersion(args.workflowId, args.version, userId);
      case 'listAppWorkflows':
        return await listAppWorkflows(args.appId, userId);
      default:
...
async function updateWorkflow(input: any, userId: string): Promise<any> {
  // Verify org access first
  const existing = await getWorkflow(input.workflowId, userId);
  if (!existing) {
...
async function deleteWorkflow(workflowId: string, userId: string): Promise<any> {
...
async function publishWorkflow(workflowId: string, userId: string): Promise<any> {
  const workflow = await getWorkflow(workflowId, userId);
  if (!workflow) {
    throw new Error('Workflow not found');
  }
...
async function exportWorkflow(workflowId: string, userId: string): Promise<any> {
  const workflow = await getWorkflow(workflowId, userId);
  if (!workflow) {
    throw new Error('Workflow not found');
  }
...
async function listAppWorkflows(appId: string, userId: string): Promise<any[]> {
  // Get the app first
  const appResult = await docClient.send(new GetCommand({
    TableName: APPS_TABLE,
    Key: { appId },
