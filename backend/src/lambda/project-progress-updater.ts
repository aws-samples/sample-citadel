...
export const handler = async (event: ProgressUpdateEvent) => {
  ...
  annotateFromCarried({
    ...carried,
    correlationId: event.detail?.sessionId,
    sessionId: event.detail?.sessionId,
  });
  console.log(
    JSON.stringify({
      level: "info",
      message: "project-progress-updater received event",
      sessionId: event.detail?.sessionId,
  ...
  const { executed } = await idempotencyGuard.withIdempotency(
    ...
    async () => {
      ...
      if (completionPercentage < 0) {
        console.log(
          `Ignoring negative progress signal: ${phase}=${completionPercentage}%`,
        );
        return;
      }
      ...
      if (phase === "implementation")
        ...
        currentPhase =
          pct === 100 ? "PLANNING_COMPLETE" : "PLANNING_IN_PROGRESS";
      else if (phase === "design")
        currentPhase = pct === 100 ? "DESIGN_COMPLETE" : "DESIGN_IN_PROGRESS";
      else if (phase === "assessment")
        currentPhase =
          pct === 100 ? "ASSESSMENT_COMPLETE" : "ASSESSMENT_IN_PROGRESS";
        ...
        try {
          await client.send(
            new UpdateCommand({
              ...
              Key: { id: sessionId },
              UpdateExpression:
                "SET #progress.#field = :pct, #progress.#cpn = :cp, updatedAt = :now",
        ...
        try {
          await client.send(
            new UpdateCommand({
              ...
              ConditionExpression:
                "attribute_exists(id) AND attribute_not_exists(#progress)",
      ...
      if (outcome === "stale") {
        console.log(
          `Skipping stale progress: ${phase}=${pct}% (already higher)`,
        );
        return;
      }
      if (outcome === "no_map") {
        console.log(
          `No project row for ${sessionId}, skipping progress update`,
        );
        return;
      }
      ...
      console.log(
        `Updated ${sessionId}: ${phase}=${pct}%, overall=${overall}%, currentPhase=${currentPhase}`,
      );
    },
  );
