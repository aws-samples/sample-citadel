 ...
 */
export function groupByAppAndHour(
  entries: AccessLogEntry[],
): Record<string, AccessLogEntry[]> {
...
export async function processMetricsEvent(
  ...
  for (const [key, groupEntries] of Object.entries(groups)) {
    ...
    const newEntries = groupEntries.filter(
      (e) => !processedIds.has(e.requestId),
    );
  ...
  const result = await deps.docClient.send(
    new QueryCommand({
      ...
      KeyConditionExpression:
        "groupId = :gid AND sortId BETWEEN :startSort AND :endSort",
      ExpressionAttributeValues: {
        ":gid": `APP#${appId}`,
...
/**
 * Aggregates dashboard metrics across every app belonging to the caller's
 * org (finding 615aa5bb — previously a table-wide ScanCommand that ignored
 * orgId entirely and aggregated every organization's apps).
 *
 * Org scoping: resolves the org's own appIds via AppsTable.OrgIndex — the
 * same GSI listApps/listDataStores already query by orgId (see
 * registry-agent-record-resolver.ts's listApps and datastore-resolver.ts's
 * listDataStores) — then aggregates only those apps' METRICS# rows via
 * GroupIndex (the same per-app query getAppMetrics uses above). No
 * table-wide scan remains.
 *
 * Requirements: 6.4, 6.5, 6.6
 */
export async function getDashboardMetrics(
  ...
  // 1. Resolve the caller org's own appIds — never a cross-org lookup.
  const orgAppsResult = await deps.docClient.send(
    new QueryCommand({
      TableName: deps.appsTable,
      IndexName: "OrgIndex",
  ...
  const appIds = (orgAppsResult.Items || [])
    .map((item) => item.appId as string)
  ...
  // 2. Aggregate METRICS# rows for only those apps (per-app GroupIndex
  // query, same shape as getAppMetrics above), run in parallel.
  const startBucket = toHourBucket(startTime);
  const endBucket = toHourBucket(endTime);
  ...
  const items = perAppResults.flatMap((r) => r.Items || []);
  ...
  for (const item of items) {
    ...
    const errors =
      ((item.clientErrorCount as number) || 0) +
      ((item.serverErrorCount as number) || 0);
...
export const handler = async (event: LogsSubscriptionEvent): Promise<void> => {
  ...
  const messages: string[] = logData.logEvents.map(
