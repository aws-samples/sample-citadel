...
export const SEED_BLUEPRINTS: SeedBlueprint[] = [
  // 1. Sequential Agent Pipeline — 3 nodes in series
  {
    ...
    description: 'Three agents executing in sequence — each passes output to the next.',
    category: 'pipeline',
    definition: {
      nodes: [
  ...
  // 2. Parallel Fan-Out — 1 root → 3 parallel → 1 convergence (5 nodes, 6 edges)
  {
    ...
    description: 'One root agent fans out to three parallel agents, then converges to a single aggregator.',
    category: 'parallel',
    definition: {
      nodes: [
  ...
  // 3. Conditional Router — 1 root → 2 conditional branches → 1 convergence (4 nodes, 4 edges)
  {
    ...
    description: 'One root agent routes to two conditional branches based on output, then converges.',
  ...
  // 4. Data Processing Pipeline — ingest → transform → validate → store (4 nodes, 3 edges)
  {
    ...
    description: 'Ingest → Transform → Validate → Store pipeline for data processing workflows.',
