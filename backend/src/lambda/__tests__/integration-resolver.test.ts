...
describe('Integration Resolver - Unit Tests', () => {
  ...
  describe('P2.A: createIntegration MCP_SERVER credential-provider wiring', () => {
    ...
    test('MCP_SERVER + OAUTH2 + CLIENT_CREDENTIALS persists credentialProviderArn and agentCoreCallbackUrl', async () => {
      ...
      bedrockAgentMock.on(CreateGatewayTargetCommand).resolves({ targetId: mockTargetId });
      secretsMock.onAnyCommand().resolves({ ARN: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-2lo' });
      ssmMock.onAnyCommand().resolves({});
      dynamoMock.onAnyCommand().resolves({});
      ...
      expect(provisionCall.endpoints.tokenEndpoint).toBe('https://idp.example.com/oauth/token');
      ...
      const createCalls = bedrockAgentMock.commandCalls(CreateGatewayTargetCommand);
      expect(createCalls.length).toBe(0);
      const eventCalls = eventBridgeMock.commandCalls(PutEventsCommand);
      expect(eventCalls.length).toBe(1);
      const detail = JSON.parse(eventCalls[0].args[0].input.Entries![0].Detail!);
      expect(detail.credentialProviderArn).toMatch(/oauth2-credential-provider/);
      ...
      expect(persisted.credentialProviderArn).toMatch(/oauth2-credential-provider/);
      expect(persisted.agentCoreCallbackUrl).toBe('https://agentcore.aws/oauth/callback/test');
      ...
      expect(result.credentialProviderArn).toMatch(/oauth2-credential-provider/);
      expect(result.agentCoreCallbackUrl).toBe('https://agentcore.aws/oauth/callback/test');
    });
    ...
    test('MCP_SERVER + OAUTH2 + AUTHORIZATION_CODE persists PENDING + emits event with OAUTH2 type', async () => {
      ...
      secretsMock.onAnyCommand().resolves({ ARN: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-3lo' });
      ssmMock.onAnyCommand().resolves({});
      dynamoMock.onAnyCommand().resolves({});
    ...
    test('MCP_SERVER + OAUTH2 + discoveryUrl uses discovered endpoints', async () => {
      ...
      bedrockAgentMock.on(CreateGatewayTargetCommand).resolves({ targetId: 'target-mcp-disc' });
      secretsMock.onAnyCommand().resolves({ ARN: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-disc' });
      ssmMock.onAnyCommand().resolves({});
      dynamoMock.onAnyCommand().resolves({});
      ...
      const provisionCall = upsertOauth2Mock.mock.calls[0][0];
      expect(provisionCall.endpoints.tokenEndpoint).toBe('https://idp.example.com/discovered/token');
      expect(provisionCall.endpoints.authorizationEndpoint).toBe('https://idp.example.com/discovered/authorize');
    });

    test('MCP_SERVER + API_KEY provisions API_KEY provider with no callbackUrl', async () => {
      bedrockAgentMock.on(CreateGatewayTargetCommand).resolves({ targetId: 'target-mcp-api' });
      secretsMock.onAnyCommand().resolves({ ARN: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-api' });
      ssmMock.onAnyCommand().resolves({});
      dynamoMock.onAnyCommand().resolves({});
      ...
      const event = {
        ...
        arguments: {
          input: {
            ...
            credentials: { authMethod: 'OAUTH2', clientId: 'cid', clientSecret: 'csec' },
            config: { serverUrl: 'https://mcp.example.com' },
          },
        },
        identity: { username: 'tester' },
    ...
    test('returns authorizationUrl for AUTHORIZATION_CODE integration in CREATE_PENDING_AUTH', async () => {
      ...
      dynamoMock.on(QueryCommand).resolves({
        Items: [{
          ...
          credentialProviderArn: 'arn:aws:bedrock-agentcore:us-east-1:111:oauth2-credential-provider/integration-mcp-3lo-existing-oauth',
          ...
          secretArn: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-3lo-existing',
          ssmParameterPrefix: '/citadel/integrations/org-3lo/mcp_server-mcp-3lo-existing',
          config: { serverUrl: 'https://mcp.example.com' },
          metadata: { version: '1.0', protocol: 'MCP', provider: 'External', authMethod: 'CONFIGURABLE' },
        }],
      });
      dynamoMock.on(PutCommand).resolves({});
    ...
    test('emits disconnect event with full detail and marks DDB DELETING', async () => {
      ...
      dynamoMock.on(QueryCommand).resolves({
        Items: [{
          ...
          credentialProviderArn: 'arn:aws:bedrock-agentcore:us-east-1:111:oauth2-credential-provider/integration-mcp-del-happy-oauth',
          credentialProviderType: 'OAUTH2',
          secretArn: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-del-happy',
          ssmParameterPrefix: '/citadel/integrations/org-del/mcp_server-mcp-del-happy',
          config: {},
          metadata: { version: '1.0', protocol: 'MCP', provider: 'External', authMethod: 'CONFIGURABLE' },
        }],
      });
      ...
      expect(bedrockAgentMock.commandCalls(DeleteGatewayTargetCommand).length).toBe(0);
      ...
      expect(detail.credentialProviderArn).toMatch(/oauth2-credential-provider/);
      expect(detail.credentialProviderType).toBe('OAUTH2');
      expect(detail.secretArn).toBe('arn:aws:secretsmanager:us-east-1:111:secret:mcp-del-happy');
    ...
    test('EventBridge publish failure surfaces as a thrown error and skips DDB update', async () => {
      ...
      dynamoMock.on(QueryCommand).resolves({
        Items: [{
          ...
          credentialProviderArn: 'arn:aws:bedrock-agentcore:us-east-1:111:oauth2-credential-provider/integration-mcp-del-tgt-fail-oauth',
          credentialProviderType: 'OAUTH2',
          secretArn: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-del-tgt-fail',
          ssmParameterPrefix: '/citadel/integrations/org-del/mcp_server-mcp-del-tgt-fail',
          config: {},
          metadata: { version: '1.0', protocol: 'MCP', provider: 'External', authMethod: 'CONFIGURABLE' },
        }],
      });
      eventBridgeMock.on(PutEventsCommand).rejects(new Error('EventBridge unreachable'));
    ...
    test('legacy integration without credentialProviderArn emits event without provider fields', async () => {
      ...
      dynamoMock.on(QueryCommand).resolves({
        Items: [{
          ...
          secretArn: 'arn:aws:secretsmanager:us-east-1:111:secret:mcp-del-legacy',
          ssmParameterPrefix: '/citadel/integrations/org-del/mcp_server-mcp-del-legacy',
          config: {},
          metadata: { version: '1.0', protocol: 'MCP', provider: 'External', authMethod: 'CONFIGURABLE' },
        }],
      });
