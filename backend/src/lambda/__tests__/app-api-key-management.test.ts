...
describe("createAppApiKey", () => {
  test("generates a 32-byte key and stores HMAC-SHA-256(pepper) hash as APIKEY#{keyId}", async () => {
    // No existing keys
    ...
    // Verify the stored hash matches HMAC-SHA-256(plaintext, pepper) and is tagged
    const expectedHash = hashApiKey(result.plaintext, TEST_PEPPER);
    const putCalls = ddbMock.commandCalls(PutCommand);
    expect(putCalls.length).toBeGreaterThanOrEqual(1);
    const storedItem = putCalls[0].args[0].input.Item!;
    expect(storedItem.hashedKey).toBe(expectedHash);
    expect(storedItem.hashAlg).toBe(HASH_ALG);
  ...
  test("key item has all required fields: keyId, name, hashedKey, prefix, status, createdAt", async () => {
    ...
    expect(new Date(storedItem.createdAt).toISOString()).toBe(
  ...
  test("expiresIn sets expiresAt timestamp correctly", async () => {
    ...
    expect(expiresAtMs).toBeGreaterThanOrEqual(
      before + expiresInSeconds * 1000 - 1000,
    );
    expect(expiresAtMs).toBeLessThanOrEqual(
      after + expiresInSeconds * 1000 + 1000,
    );
  });
...
describe("rotateAppApiKey", () => {
  ...
  test("new key has different keyId and hashedKey than old key", async () => {
    ...
    const newHash = hashApiKey(result.newKey.plaintext, TEST_PEPPER);
  ...
  test("transaction contains both Put (new key) and Update (revoke old)", async () => {
    ...
    // New key is written with the current HMAC-with-pepper algorithm tag
    expect(putItem!.Put!.Item!.hashAlg).toBe(HASH_ALG);
