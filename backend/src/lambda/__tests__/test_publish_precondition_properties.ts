...
describe('Property 1: Publish precondition validation', () => {
    ...
    return fc.assert(
      fc.property(
        ...
        (agents, workflowIds, config) => {
          ...
          const hasWorkflow = workflowIds.length >= 1;
          const hasAdminEmail = !!config.adminEmail;
   ...
   */
  it('error list mentions workflow requirement when none bound', () => {
    return fc.assert(
      fc.property(
        agentBindingsArb.filter(agents => agents.every(a => a.status === 'READY')),
        (agents) => {
          const metadata: AppMetadata = {
            appId: 'app-test',
            name: 'Test App',
