...
describe('validatePublishPreconditions (extended)', () => {
  ...
  test('returns error when no workflows are bound', () => {
  ...
  test('returns error when workflowIds is undefined', () => {
  ...
  test('returns error when admin email is missing from config values', () => {
    const metadata = makeMetadata();
    const components: ComponentItem[] = [
      makeAgentBinding('agent-1', 'READY'),
      makeConfigValues({ apiKey: 'sk-test' }),
    ];
    const errors = validatePublishPreconditions(metadata, components);
    expect(errors).toContainEqual(expect.stringContaining('Admin email'));
  });

  test('returns error when no config values exist at all', () => {
    const metadata = makeMetadata();
    const components: ComponentItem[] = [
      makeAgentBinding('agent-1', 'READY'),
    ];
  ...
  test('returns all failing preconditions in a single call', () => {
    ...
    expect(errors.some(e => e.includes('Admin email'))).toBe(true);
  });
  ...
  test('handles empty component items array', () => {
    const metadata = makeMetadata({ workflowIds: ['wf-1'] });
    const components: ComponentItem[] = [];
    const errors = validatePublishPreconditions(metadata, components);
    expect(errors).toContainEqual(expect.stringContaining('Admin email'));
...
describe('publishApp', () => {
  ...
  test('throws structured error when preconditions fail', async () => {
    ddbMock.on(QueryCommand).resolves({
      Items: [
        makeMetadata({ workflowIds: [] }), // no workflows
      ],
    });
