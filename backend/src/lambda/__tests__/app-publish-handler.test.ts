...
describe("validatePublishPreconditions (extended)", () => {
  ...
  test("admin email is optional — does NOT block publish when missing", () => {
    ...
    expect(
      errors.some(
        (e) => e.includes("Admin email") || e.includes("admin email"),
      ),
    ).toBe(false);
  });
  ...
  test("publishes cleanly when only an agent is bound and there is no config", () => {
    ...
    const metadata = makeMetadata();
    const components: ComponentItem[] = [makeAgentBinding("agent-1", "READY")];
    const errors = validatePublishPreconditions(metadata, components);
    expect(errors).toEqual([]);
  });
  ...
  test("returns error when config schema exists but values are missing", () => {
    ...
    expect(errors).toContainEqual(
      expect.stringContaining("Configuration values are required"),
    );
  });
  ...
  test("returns all failing preconditions in a single call", () => {
    ...
    expect(
      errors.some((e) => e.includes("Configuration values are required")),
    ).toBe(true);
  ...
  test("does not mutate the metadata object on precondition failure", () => {
    ...
    const metadataCopy = {
      ...metadata,
      workflowIds: [...metadata.workflowIds],
    };
...
describe("provisionApiGateway", () => {
  ...
  test("calls SDK commands in correct sequence: CreateApi → CreateStage → CreateIntegration → CreateAuthorizer → CreateRoute", async () => {
    ...
    await provisionApiGateway(
      "app-1",
      "dev",
      "arn:aws:lambda:us-east-1:123:function:auth",
      "citadel-agents-dev",
      "us-east-1",
    );
  ...
  test("API name follows citadel-app-{appId}-{env} convention", async () => {
    ...
    expect(createApiCall.args[0].input.Name).toBe(
      "citadel-app-my-app-42-staging",
    );
  ...
  test("returns apiId and derived endpointUrl", async () => {
    const result = await provisionApiGateway(
      "app-1",
      "dev",
      "arn:auth",
      "bus",
    ...
    expect(result.endpointUrl).toBe(
      "https://api-123.execute-api.us-west-2.amazonaws.com",
    );
  });

  test("rolls back by calling DeleteApiCommand on CreateStage failure", async () => {
    apiGwMock
      .on(CreateStageCommand)
      .rejects(new Error("Stage creation failed"));
  ...
  test("rolls back by calling DeleteApiCommand on CreateIntegration failure", async () => {
    apiGwMock
      .on(CreateIntegrationCommand)
      .rejects(new Error("Integration failed"));
  ...
  test("creates CloudWatch log group and configures access logging with structured JSON format", async () => {
    ...
    expect(logGroupCalls[0].args[0].input.logGroupName).toBe(
      "/aws/apigateway/citadel-app-app-1-dev",
    );
    ...
    expect(input.AccessLogSettings!.DestinationArn).toContain(
      "/aws/apigateway/citadel-app-app-1-dev",
    );
...
describe("publishApp", () => {
  ...
  test("idempotency: already-published app returns current state without re-provisioning", async () => {
    ...
    expect(result.endpointUrl).toBe(
      "https://api-existing.execute-api.us-east-1.amazonaws.com",
    );
  ...
  test("full publish flow: provisions API GW, creates key, updates status, emits event", async () => {
    ...
    expect(result.app.status).toBe("PUBLISHED");
    expect(result.endpointUrl).toBe(
      "https://api-123.execute-api.us-east-1.amazonaws.com",
    );
    ...
    const keyPut = putCalls.find((c) =>
      c.args[0].input.Item?.sortId?.startsWith("APIKEY#"),
    );
    ...
    const metaUpdate = updateCalls.find(
      (c) =>
        (c.args[0].input.Key as Record<string, unknown> | undefined)?.appId ===
          "app-1" &&
        (c.args[0].input.Key as Record<string, unknown> | undefined)?.sortId ===
          undefined &&
        (
    ...
    const metaValues = metaUpdate!.args[0].input
...
describe("buildStatusTransitionEvent", () => {
  describe("ACTIVE → PUBLISHED event", () => {
    ...
    test("includes endpointUrl and apiKeyId for ACTIVE→PUBLISHED", () => {
      const event = buildStatusTransitionEvent("active", "published", detail);
      expect(event.detail.endpointUrl).toBe(
        "https://api-123.execute-api.us-east-1.amazonaws.com",
      );
...
describe("publishApp intake progress-on-publish", () => {
  ...
  test("emits NO intake progress event when the app has no sourceProjectId linkage (backward compatible)", async () => {
    ...
    expect(ebMock.commandCalls(PutEventsCommand).length).toBeGreaterThanOrEqual(
      1,
    );
  });
  ...
  test("publish still succeeds when the progress emission fails (best-effort)", async () => {
    ...
    ebMock
      .on(PutEventsCommand)
      .callsFake((input: { Entries?: Array<{ Source?: string }> }) => {
        if (
          input.Entries?.some((e) => e.Source === "agent_intake.implementation")
        ) {
