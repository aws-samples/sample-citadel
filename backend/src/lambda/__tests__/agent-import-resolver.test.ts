...
jest.mock("../../services/agent-discovery", () => {
  ...
  return {
    ...
    candidateFromManifest: (...args: unknown[]) =>
      mockCandidateFromManifest(...args),
  };
});
...
jest.mock("../registry-agent-authority-lifecycle", () => ({
  __esModule: true,
  grantFabricatorAuthority: (...args: unknown[]) =>
    mockGrantFabricatorAuthority(...args),
}));
...
jest.mock("../../utils/governance-flag", () => ({
  __esModule: true,
  getGovernanceEnforce: (...args: unknown[]) =>
    mockGetGovernanceEnforce(...args),
}));
...
jest.mock("../../utils/credential-manager", () => ({
  __esModule: true,
  storeAgentInvocationSecret: (...args: unknown[]) =>
    mockStoreAgentInvocationSecret(...args),
}));
...
const SOURCE_ARN =
  "arn:aws:bedrock-agentcore:us-east-1:111122223333:runtime/pay";
...
const ANALYSIS_ROLE_ARN =
  "arn:aws:iam::111122223333:role/citadel-analysis-readonly";
...
const CROSS_ACCOUNT_INVOKE_ROLE_ARN =
  "arn:aws:iam::999988887777:role/citadel-invoke-cross";
const eventWithOrg = {
...
beforeEach(() => {
  ...
  // This is also the safe default for the OTHER admin/architect gates in this
  // file (attestAgentImport, discovery queries) — tests that need an
  // authorized caller set these explicitly.
...
describe("importAgent — create (no existing match)", () => {
  // Authorized by default (decision ff48d6f9, finding 234cda06): these tests
  // exercise create/write logic, not the role gate itself.
  beforeEach(() => {
  ...
  it("serializes metadata with state inactive, external ownership, and event-derived orgId", async () => {
    ...
    const metaArg = mockSerializeCustomMetadata.mock.calls[0][0] as {
      orgId: string;
    };
  ...
  it("passes a stringified customMetadata to createResource", async () => {
    ...
    const payload = mockCreateResource.mock.calls[0][2] as {
...
describe("importAgent — guards", () => {
  it("throws when the caller organization cannot be determined", async () => {
    ...
    await expect(importAgent(validInput(), eventWithOrg)).rejects.toThrow(
      /organization/i,
    );
  ...
  it("throws listing invocation.protocol when the invocation block is invalid", async () => {
    ...
    await expect(importAgent(bad, eventWithOrg)).rejects.toThrow(
      /invocation\.protocol/,
    );
    expect(mockCreateResource).not.toHaveBeenCalled();
  });
...
// ── importAgent: role gate (decision ff48d6f9 / finding 234cda06) ────────
// importAgent is gated like its sibling agent-import mutations (attest/test/
// probe/probeReachability/publishGateway/unpublishGateway/proposeTier3/
// acceptTier3): admin or architect only, checked AFTER the required-org check
// and BEFORE any write.
describe("importAgent — role gate (decision ff48d6f9, finding 234cda06)", () => {
  it("rejects a non-architect non-admin org member before any write", async () => {
    mockIsAdminFromEvent.mockReturnValue(false);
    mockHasRoleFromEvent.mockReturnValue(false);
  ...
  it("allows an architect caller through to the write path", async () => {
    mockIsAdminFromEvent.mockReturnValue(false);
    mockHasRoleFromEvent.mockImplementation(
      (_e: unknown, role: string) => role === "architect",
    );
    mockCreateResource.mockResolvedValue(
      existingRecord({
        recordId: "rec-arch",
    ...
    expect(result).toEqual(expect.objectContaining({ agentId: "rec-arch" }));
    expect(mockCreateResource).toHaveBeenCalledTimes(1);
  });

  it("allows an admin caller through to the write path", async () => {
    mockIsAdminFromEvent.mockReturnValue(true);
    mockHasRoleFromEvent.mockReturnValue(false);
    mockCreateResource.mockResolvedValue(
      existingRecord({
    ...
    expect(result).toEqual(expect.objectContaining({ agentId: "rec-admin" }));
    expect(mockCreateResource).toHaveBeenCalledTimes(1);
  });

  it("still rejects a role-less caller missing an org (org check runs first)", async () => {
    mockExtractOrgFromEvent.mockResolvedValue(null);
    mockIsAdminFromEvent.mockReturnValue(false);
    mockHasRoleFromEvent.mockReturnValue(false);
  ...
  it("handler: routes importAgent and rejects a non-privileged caller with the same message", async () => {
    mockIsAdminFromEvent.mockReturnValue(false);
    mockHasRoleFromEvent.mockReturnValue(false);
...
describe("importAgent — conflict resolution", () => {
  ...
  it('onConflict="link" returns the existing mapped record with no create/update', async () => {
    ...
    const result = await importAgent(
      validInput({ onConflict: "link" }),
      eventWithOrg,
    );

    expect(mockMapToAgentConfig).toHaveBeenCalledWith(existing);
    expect(result).toEqual(
      expect.objectContaining({ agentId: "rec-existing" }),
    );
    expect(mockCreateResource).not.toHaveBeenCalled();
    expect(mockUpdateResource).not.toHaveBeenCalled();
  });
  ...
  it('onConflict="copy" creates a record with a suffixed name', async () => {
    ...
    mockCreateResource.mockResolvedValue(
      existingRecord({
        recordId: "rec-copy",
        name: "PaymentsAgent (imported copy)",
        sourceArn: SOURCE_ARN,
      }),
    );
    ...
    expect(mockCreateResource).toHaveBeenCalledTimes(1);
    expect(mockCreateResource.mock.calls[0][1]).toBe(
      "PaymentsAgent (imported copy)",
    );
  });
});
...
describe("importAgent — invocation auth-secret storage", () => {
  ...
  const stampedMeta = ():
    | { invocation?: { auth?: { secretRef?: string; mode?: string } } }
    | undefined => {
  ...
  it("stores a raw invocationSecret exactly once with the raw value and the caller org", async () => {
    ...
    await importAgent(
      withAuth({ mode: "API_KEY" }, { invocationSecret: RAW_SECRET }),
      eventWithOrg,
    );

    expect(mockStoreAgentInvocationSecret).toHaveBeenCalledTimes(1);
    const args = mockStoreAgentInvocationSecret.mock.calls[0] as [
      string,
      string,
      string,
    ];
  ...
  it("NEVER persists the raw secret in customMetadata, description, or any createResource arg", async () => {
    ...
    const serializedMeta = mockSerializeCustomMetadata.mock.results[0]
      .value as string;
    ...
    expect(JSON.stringify(mockCreateResource.mock.calls[0])).not.toContain(
      RAW_SECRET,
    );
  });
  ...
  it("uses a pre-existing invocationSecretRef as-is and does NOT call the secret store", async () => {
    ...
    await importAgent(
      withAuth({ mode: "API_KEY", secretRef: "preexisting-ref-1" }),
      eventWithOrg,
    );

    expect(mockStoreAgentInvocationSecret).not.toHaveBeenCalled();
    expect(stampedMeta()?.invocation?.auth?.secretRef).toBe(
      "preexisting-ref-1",
    );
  });
  ...
  it("fails the import (no record created) and does not leak the raw secret when the store throws", async () => {
    mockStoreAgentInvocationSecret.mockRejectedValue(
  ...
  it("stores the secret and sets the ref on the REPLACE path", async () => {
    ...
    await importAgent(
      withAuth(
        { mode: "API_KEY" },
        { onConflict: "replace", invocationSecret: RAW_SECRET },
      ),
      eventWithOrg,
    );
  ...
  it("stores the secret and sets the ref on the COPY path", async () => {
    ...
    await importAgent(
      withAuth(
        { mode: "API_KEY" },
        { onConflict: "copy", invocationSecret: RAW_SECRET },
      ),
      eventWithOrg,
    );
  ...
  it("does NOT store a secret on a no-op link (no record is created)", async () => {
    ...
    await importAgent(
      withAuth(
        { mode: "API_KEY" },
        { onConflict: "link", invocationSecret: RAW_SECRET },
      ),
      eventWithOrg,
    );
  ...
  it("buildImportDescriptor carries invocationSecret as a transient top-level field, never in invocation/origin", () => {
    ...
    expect((d.invocation as { auth: Record<string, unknown> }).auth).toEqual({
      mode: "API_KEY",
    });
  });
});
...
describe("importAgent — invocation analysisRoleArn", () => {
  ...
  it("stamps invocation.analysisRoleArn into the persisted customMetadata when provided", async () => {
    ...
    await importAgent(
      validInput({ invocationAnalysisRoleArn: ANALYSIS_ROLE_ARN }),
      eventWithOrg,
    );
  ...
  it("handler: a flat invocationAnalysisRoleArn flows end-to-end into persisted invocation.analysisRoleArn", async () => {
    ...
    await handler({
      info: { fieldName: "importAgent" },
      arguments: {
        input: validFlatInput({ invocationAnalysisRoleArn: ANALYSIS_ROLE_ARN }),
      },
      identity: eventWithOrg.identity,
    });
...
describe("importAgent — invocation roleArn / externalId", () => {
  ...
  /** The invocation block within the metadata handed to serializeCustomMetadata. */
  const stampedInvocation = ():
    { roleArn?: string; externalId?: string } | undefined => {
    const call = mockSerializeCustomMetadata.mock.calls[0];
    return call
      ? (call[0] as { invocation?: { roleArn?: string; externalId?: string } })
          .invocation
      : undefined;
  };
...
describe("importAgent — governance attestation", () => {
  ...
  it("still returns the created agent when grantFabricatorAuthority throws (best-effort, logged)", async () => {
    ...
    mockGrantFabricatorAuthority.mockRejectedValue(
      new Error("authority-units table down"),
    );
    const errSpy = jest.spyOn(console, "error").mockImplementation(() => {});
  ...
  it("stamps attestation and grants authority on copy (new recordId)", async () => {
    ...
    expect(stampedMeta()).toEqual(
      expect.objectContaining({
        governanceAttestation: expect.objectContaining({ status: "pending" }),
      }),
    );
    expect(mockGrantFabricatorAuthority).toHaveBeenCalledTimes(1);
...
describe("importAgent — ADR-on-import", () => {
  ...
  it("invokes createADR with a SYSTEM principal — not the importing caller (no architect/admin required)", async () => {
    ...
    expect(mockCreateADR).toHaveBeenCalledTimes(1);
    const authCtx = mockCreateADR.mock.calls[0][1] as {
      userId: string;
      roles?: string[];
    };
...
describe("buildImportCopyName", () => {
  it('returns "<name> (imported copy)" when there is no collision', () => {
    expect(buildImportCopyName("Agent", new Set())).toBe(
      "Agent (imported copy)",
    );
  });
...
describe("buildImportDescriptor", () => {
  ...
  it("parses an AWSJSON manifest delivered as a JSON string", () => {
    const d = buildImportDescriptor(
      validFlatInput({ manifest: '{"name":"X"}' }),
    );
  ...
  it("lowercases the ImportConflictPolicy enum (REPLACE → replace)", () => {
    expect(
      buildImportDescriptor(validFlatInput({ onConflict: "REPLACE" }))
        .onConflict,
    ).toBe("replace");
  ...
  it("maps invocationAuthHeader into invocation.auth.header (and omits it when absent)", () => {
    ...
    expect(
      (withHeader.invocation as { auth: Record<string, unknown> }).auth,
    ).toEqual({
      mode: "API_KEY",
      header: "x-api-key",
    });
    // Absent → no `header` key at all (back-compat: existing imports unaffected).
    const withoutHeader = buildImportDescriptor(
      validFlatInput({ invocationAuthMode: "API_KEY" }),
    );
    expect(
      "header" in
        (withoutHeader.invocation as { auth: Record<string, unknown> }).auth,
    ).toBe(false);
  });
  ...
  it("carries invocationAnalysisRoleArn as a top-level descriptor field (and omits when absent)", () => {
    ...
    expect(
      (withArn.invocation as { analysisRoleArn?: string }).analysisRoleArn,
    ).toBeUndefined();
    // Absent ⇒ no top-level key at all (back-compat: existing imports unaffected).
    expect(
      "invocationAnalysisRoleArn" in buildImportDescriptor(validFlatInput()),
    ).toBe(false);
  });
  ...
  it("carries invocationRoleArn + invocationExternalId as top-level descriptor fields (and omits when absent)", () => {
    ...
    // Not pre-nested into invocation at the mapping layer — importAgent stamps it.
    const inv = withRole.invocation as {
      roleArn?: string;
      externalId?: string;
    };
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ token, name }) => {
          ...
          mockExtractOrgFromEvent.mockResolvedValue(ORG);
          mockSerializeCustomMetadata.mockImplementation((m: unknown) =>
            JSON.stringify(m),
          );
          ...
          mockMapToAgentConfig.mockImplementation(
            (r: { recordId: string }) => ({
              agentId: r.recordId,
            }),
          );
...
describe("handler — discoverAgents", () => {
  ...
  it("SCAN threads discoveryRoleArn/discoveryExternalId into the tagScanDiscover scope", async () => {
    ...
    const res = (await handler(
      authedEvent({
        ...
        tagValue: "true",
        discoveryRoleArn:
          "arn:aws:iam::222233334444:role/citadel-readonly-discovery",
  ...
  it("PASTE builds a single flat candidate from a resolved ARN ref", async () => {
    ...
    const res = (await handler(
      authedEvent({ source: "PASTE", ref: arn }),
    )) as FlatAgentCandidate[];
  ...
  it("PASTE sets sourceArn/region/account to null for a non-ARN (URL) ref", async () => {
    ...
    const res = (await handler(
      authedEvent({ source: "PASTE", ref: url }),
    )) as FlatAgentCandidate[];
  ...
  it("throws when PASTE is missing its ref", async () => {
    await expect(handler(authedEvent({ source: "PASTE" }))).rejects.toThrow(
      /ref is required/i,
    );
  ...
  it("MANIFEST maps the candidate from candidateFromManifest to the flat shape", async () => {
    ...
    expect(mockCandidateFromManifest).toHaveBeenCalledWith(
      '{"name":"ManifestAgent"}',
    );
    expect(res).toEqual([
      {
  ...
  it("surfaces InvalidSourceRefError from a bad PASTE ref as a clean GraphQL error", async () => {
    ...
    await expect(
      handler(authedEvent({ source: "PASTE", ref: "nonsense" })),
    ).rejects.toThrow(/unrecognised source ref/);
  });
  ...
  it("rejects an unrecognised discovery source", async () => {
    await expect(handler(authedEvent({ source: "BOGUS" }))).rejects.toThrow(
...
describe("handler — discoverAgents authorization", () => {
  ...
  it("allows a caller with role architect (reaches discovery logic)", async () => {
    ...
    expect(mockHasRoleFromEvent).toHaveBeenCalledWith(
      expect.anything(),
      "architect",
    );
  });
...
describe("handler — describeAgentCandidate authorization", () => {
  ...
  it("allows a caller with role admin (reaches the protocol adapter)", async () => {
    ...
    await expect(handler(describeEvent())).resolves.toBe(
      JSON.stringify({ name: "pay" }),
    );
  ...
  it("denies a caller with role developer (authenticated but neither admin nor architect)", async () => {
    ...
    await expect(handler(describeEvent())).rejects.toThrow(
      /admin or architect/i,
    );
    expect(mockResolveSourceRef).not.toHaveBeenCalled();
    expect(mockDescribe).not.toHaveBeenCalled();
  });
...
describe("import lifecycle events", () => {
  ...
  beforeEach(() => {
    ...
    mockResolveAdapter.mockReturnValue({ describe: mockDescribe });
    mockDescribe.mockReset();
    // Authorized by default: the registered/discovered sub-describes below
    // exercise importAgent/discoverAgents success paths, not the role gates
    // (decision ff48d6f9, finding 234cda06 for importAgent). Individual tests
    // needing denial (none currently in this block) would override locally.
    mockIsAdminFromEvent.mockReturnValue(true);
  });
  ...
  describe("agent.import.discovered", () => {
    it("emits once per discovery call with source, candidateCount, substrates, correlationId, timestamp", async () => {
      mockIsAdminFromEvent.mockReturnValue(true);
      const scannedArn =
        "arn:aws:lambda:us-east-1:111122223333:function:scanned";
      mockTagScanDiscover.mockResolvedValue([
        {
          origin: {
            sourceArn: scannedArn,
  ...
  describe("agent.import.failed", () => {
    it('emits operation="import" and still propagates the original error', async () => {
      ...
      expect(fails[0].payload).toEqual(
    ...
    it('emits operation="discover" and still propagates the original error', async () => {
      ...
      expect(fails).toHaveLength(1);
      expect(fails[0].payload).toEqual(
        expect.objectContaining({ operation: "discover" }),
      );
    });
    ...
    it('emits operation="describe" and still propagates the original error', async () => {
      ...
      expect(fails).toHaveLength(1);
      expect(fails[0].payload).toEqual(
        expect.objectContaining({ operation: "describe" }),
      );
    });
  });
...
describe("attestAgentImport", () => {
  ...
  it("admin: flips a pending record to attested with attestedBy + attestedAt, writing once", async () => {
    ...
    mockGetResource.mockResolvedValue(
      importedAgentRecord({ status: "pending" }),
    );
    mockUpdateResource.mockResolvedValue(
    ...
    const ga = (writtenMeta()?.governanceAttestation ?? {}) as Record<
      string,
      unknown
    >;
  ...
  it("developer / no privileged role: authorization error, no write, no event", async () => {
    ...
    await expect(attestAgentImport("rec-imp", developerEvent)).rejects.toThrow(
      /unauthor/i,
    );
  ...
  it("record with no governanceAttestation: errors (nothing to attest), no write", async () => {
    ...
    await expect(attestAgentImport("rec-plain", adminEvent)).rejects.toThrow(
      /nothing to attest/i,
    );
  ...
  it("not found: clean error, no write", async () => {
    ...
    await expect(attestAgentImport("missing", adminEvent)).rejects.toThrow(
      /not found/i,
    );
  ...
  it("cross-org non-admin caller: not-found (no write, no existence leak)", async () => {
    ...
    mockGetResource.mockResolvedValue(
      importedAgentRecord({ orgId: "other-org", status: "pending" }),
    );

    await expect(attestAgentImport("rec-imp", architectEvent)).rejects.toThrow(
      /not found/i,
    );
  ...
  it("admin bypasses org-scope: attests a record owned by another org", async () => {
    ...
    expect(emittedAttested()[0].payload).toEqual(
