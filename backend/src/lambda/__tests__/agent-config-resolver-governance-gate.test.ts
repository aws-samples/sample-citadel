...
describe("agent-config-resolver governance activation gate (US-IMP)", () => {
  ...
  describe("updateAgentConfigRegistry", () => {
    ...
    test.each(["shadow", "permissive"])(
      ...
      async (mode) => {
        ...
        expect(mockSubmitForApproval).toHaveBeenCalledWith("agent-imp-1");
    ...
    test("REGRESSION: non-imported + strict → activation unchanged, gate never fires", async () => {
      ...
      expect(mockSubmitForApproval).toHaveBeenCalledWith("agent-plain-1");
      expect(mockUpdateResourceStatus).not.toHaveBeenCalledWith(
        "agent",
        "agent-plain-1",
        "APPROVED",
    ...
    test("imported + pending + strict + METADATA-ONLY (no state) → gate does NOT fire", async () => {
      mockGetResource.mockResolvedValue(
        importedRecord("pending", "PENDING_APPROVAL"),
      );
      mockUpdateResource.mockResolvedValue(
        importedRecord("pending", "PENDING_APPROVAL"),
      );
  ...
  describe("activateProjectAgents", () => {
    ...
    test("strict → imported-unattested agent lands in failed without aborting the batch", async () => {
      ...
      // Only the non-imported agent's submit-for-approval happened; the gate
      // blocked the imported one before its write.
      expect(mockUpdateResourceStatus).not.toHaveBeenCalled();
      expect(mockSubmitForApproval).toHaveBeenCalledTimes(1);
      expect(mockSubmitForApproval).toHaveBeenCalledWith("plain-1");
    });
    ...
    test("shadow → imported-unattested agent activates and emits a gate event", async () => {
      ...
      expect(mockSubmitForApproval).toHaveBeenCalledWith("imp-1");
      expect(mockUpdateResourceStatus).not.toHaveBeenCalledWith(
        "agent",
        "imp-1",
        "APPROVED",
