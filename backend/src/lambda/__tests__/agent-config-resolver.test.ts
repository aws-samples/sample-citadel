...
const mockMapToAgentConfig = jest.fn(
  (record: RegistryRecordFixture | undefined) => ({
...
// activateProjectAgents dispatch now gates on assertProjectOrgAccess (finding
// 1fcfd11e) BEFORE the (deliberately ungated, internally-reused)
// activateProjectAgents core runs. Defaults to allow so these dispatch tests
// exercise the activation logic itself; the dispatch-gate-enumeration test
// file covers rejection behaviour.
const mockAssertProjectOrgAccess = jest.fn().mockResolvedValue(undefined);
...
describe("agent-config-resolver", () => {
  ...
  describe("getAgentConfig", () => {
    ...
    test("returns null when not found", async () => {
      dynamoMock.on(GetCommand).resolves({});
      const result = await handler(
        makeEvent("getAgentConfig", { agentId: "missing" }),
      );
  ...
  describe("updateAgentConfig", () => {
    test("throws when config not found", async () => {
      ...
      await expect(
        handler(
          makeEvent("updateAgentConfig", { input: { agentId: "missing" } }),
        ),
...
describe("updateAgentConfigRegistry — lazy IAM trust-path activation (US-IMP)", () => {
  ...
  function importedRecord(
    ...
    if (opts.roleArn !== undefined) {
      meta.invocation = {
        ...
        ...(opts.analysisRoleArn !== undefined
          ? { analysisRoleArn: opts.analysisRoleArn }
          : {}),
  ...
  const activateEvent = () => ({
    ...
    // The US-IMP fixture records all carry orgId: 'org-1' — the new tenancy
    // gate (finding 1fcfd11e) reconciles the caller's org against that
    // EXISTING record's orgId before any mutation, so this identity must
    // match for these activation-gate tests to reach the code they exercise.
  ...
  test("imported+pending+roleArn+clean → auto-attests (system:trust-path) and activation proceeds", async () => {
    ...
    expect(persisted.governanceAttestation.attestedBy).toBe(
      "system:trust-path",
    );
  ...
  test("imported+pending+roleArn+findings → stays pending, findings stored, strict gate blocks", async () => {
    ...
    await expect(handler(activateEvent())).rejects.toThrow(
      "Activation blocked",
    );
  ...
  test("trust-path throws → activation not crashed, no auto-attest (permissive proceeds)", async () => {
    ...
    mockGetResource
      .mockResolvedValueOnce(importedRecord({ roleArn: ROLE_ARN }))
      .mockResolvedValueOnce(approvedRecord());
    mockComputeTrustPath.mockRejectedValue(
      new Error("cross-account role unresolvable"),
    );
  ...
  const ANALYSIS_ROLE_ARN =
    "arn:aws:iam::123456789012:role/citadel-readonly-analysis";
  ...
  test("CROSS-ACCOUNT + analysisRoleArn + clean → assumes analysis role w/ ExternalId, computeTrustPath uses x-acct client, auto-attests (crossAccount summary)", async () => {
    ...
    mockGetResource
      .mockResolvedValueOnce(
        importedRecord({
          roleArn: ROLE_ARN,
          analysisRoleArn: ANALYSIS_ROLE_ARN,
    ...
    expect(mockAssumeAnalysisRoleClient).toHaveBeenCalledWith(
      ANALYSIS_ROLE_ARN,
      EXTERNAL_ID,
    );
    expect(mockComputeTrustPath).toHaveBeenCalledWith(ROLE_ARN, {
  ...
  test("CROSS-ACCOUNT + analysisRoleArn but assume THROWS → finding recorded, pending, NOT crashed (permissive proceeds), no computeTrustPath, creds not logged", async () => {
    process.env.ACCOUNT_ID = CROSS_ACCOUNT_DEPLOYMENT;
    mockGetGovernanceEnforce.mockResolvedValue("permissive");
    mockAssumeAnalysisRoleClient.mockRejectedValue(
      new Error("AccessDenied: not authorized to sts:AssumeRole"),
    );
    mockGetResource
      .mockResolvedValueOnce(
        importedRecord({
