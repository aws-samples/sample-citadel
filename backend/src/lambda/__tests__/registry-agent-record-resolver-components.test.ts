...
function seedAppWithBinding(
  opts: { bindingStatus?: string; agentId?: string } = {},
) {
...
function seedTargetAgent(
  opts: { agentId?: string; state?: string; status?: string } = {},
) {
  const agentId = opts.agentId ?? "agent-1";
  ...
  describe('type "agent"', () => {
    test("emits app.component.added event with agent component detail", async () => {
      ...
      const added = entries.find(
        (e) => e?.DetailType === "app.component.added",
      );
...
describe("registry-agent-record-resolver — removeAppComponent", () => {
  ...
  function seedAppForRemove(
    opts: { agentBindings?: unknown[]; permissions?: unknown[] } = {},
  ) {
    seedApp({
      agentBindings: opts.agentBindings ?? [
        {
  ...
  test("emits app.component.removed event for agent type", async () => {
    ...
    const removed = entries.find(
      (e) => e?.DetailType === "app.component.removed",
    );
