...
const getGovernanceEffectiveAtMock =
  getGovernanceEffectiveAt as jest.MockedFunction<
    typeof getGovernanceEffectiveAt
  >;
...
describe("projectFinding", () => {
  ...
  test("maps traceId when present on the DDB row", () => {
    ...
    const projected = projectFinding(row);
    expect(projected.traceId).toBe("1-5f2f0000-abcdef0123456789abcdef01");
  });

  test("coerces missing traceId (pre-linking finding) to null", () => {
    const row = makeDdbRow();
    delete (row as Record<string, unknown>).traceId;
    const projected = projectFinding(row);
    expect(projected.traceId).toBeNull();
  });

  test("coerces empty-string traceId to null", () => {
...
describe("getGovernanceMode", () => {
  test("returns enforce and effectiveAt from mocked SSM helpers", async () => {
    ...
    const result = (await handler(
      makeEvent({ fieldName: "getGovernanceMode" }),
    )) as {
      enforce: string;
...
describe("getReconcilerStatus", () => {
  ...
  test("returns zero defaults when shape does not match", async () => {
    ...
    ssmMock.on(GetParameterCommand).resolves({
      Parameter: {
        Value: JSON.stringify({ lastRunAt: "x", classifications: "wrong" }),
      },
    });
...
describe("getRolloutReadiness", () => {
  ...
  test('data-1 returns PASS with detail "All N ..." when no rows are missing', async () => {
    ...
    const result = (await handler(
      ...
      checks: Array<{ id: string; status: string; detail: string | null }>;
    };
...
describe("getRolloutReadiness real checks (Wave 2.B)", () => {
  ...
  test("data-2 PASS: sample of 5 units, all return live records", async () => {
    ...
    expect(dataTwo.detail).toBe("Sampled 5 units, all live and non-deprecated");
  ...
  test("data-2 FAIL: 2 of 5 deprecated, evidence has IDs", async () => {
    ...
    expect(dataTwo.detail).toBe("2 of 5 sampled units missing or deprecated");
  ...
  test("counts after all checks: sum to 11; stubCount = 3 in the all-real-pass scenario", async () => {
    ...
    ddbMock.on(ScanCommand).callsFake((input: ScanCommandInput) => {
      if (input.TableName === "citadel-governance-ledger-test") {
        if (input.Select === "COUNT") {
          ...
          const isMismatch = String(input.FilterExpression ?? "").includes(
            ":deny",
          );
    ...
    ssmMock.on(GetParameterHistoryCommand).resolves({
      Parameters: [
        {
          Value: "permissive",
          LastModifiedDate: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
        },
        {
          Value: "shadow",
          LastModifiedDate: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000),
        },
        {
          Value: "permissive",
  ...
  test("decision filter: only deny and escalate items are included", async () => {
    ...
    expect(
      result.buckets.every(
        (b) => b.decision === "deny" || b.decision === "escalate",
      ),
    ).toBe(true);
  });
...
describe("setGovernanceMode", () => {
  ...
  function stubSsm(opts: {
    ...
    ssmMock
      ...
      .callsFake((input: GetParameterCommandInput) => {
    ...
    ssmMock
      ...
      .callsFake((input: PutParameterCommandInput) => {
        const name: string = input?.Name ?? "";
  ...
  test("admin gate throws Forbidden for non-admin", async () => {
    isAdminFromEventMock.mockReturnValue(false);
    await expect(
      handler(adminEvent({ targetMode: "shadow", acknowledgement: ACK_TEXT })),
    ).rejects.toThrow("Forbidden: admin required");
  });

  test("acknowledgement string mismatch throws", async () => {
    await expect(
      handler(adminEvent({ targetMode: "shadow", acknowledgement: "wrong" })),
    ).rejects.toThrow("Acknowledgement string mismatch");
  });

  test("invalid targetMode throws with the offending value", async () => {
    await expect(
      handler(adminEvent({ targetMode: "enforce", acknowledgement: ACK_TEXT })),
  ...
  test("permissive→strict in prod env (env=prod) THROWS Decision #8", async () => {
    ...
    await expect(
      handler(adminEvent({ targetMode: "strict", acknowledgement: ACK_TEXT })),
...
describe("markReadinessCheckVerified", () => {
  ...
  function adminVerifyEvent(
    input: {
      checkId?: string;
      expiresInDays?: number | null;
      note?: string | null;
    },
    identity: Record<string, unknown> = { sub: "verifier-sub-1" },
  ): HandlerEvent {
    return makeEvent({
  ...
  test("rejects an unrecognised id (bogus) — not in manual allowlist", async () => {
    ...
    ).rejects.toThrow("Check ID not in manual-verification allowlist: bogus");
  });
  ...
  test("default expiresInDays = 7 when omitted", async () => {
    ...
    const result = (await handler(adminVerifyEvent({ checkId: "own-1" }))) as {
      verifiedAt: string;
      expiresAt: string;
    };
  ...
  test("writes SSM with the expected name + JSON shape", async () => {
    ...
    expect(input.Name).toBe("/citadel/governance/readiness/manual/dev/own-1");
  ...
  test("SSM write failure propagates as a thrown error", async () => {
    ssmMock
      .on(PutParameterCommand)
      .rejects(new Error("SSM PutParameter denied"));
...
describe("getDecisionTrace", () => {
  ...
  // P1 test 6 (architect task 9b3f4f78) — traceId flows through
  // getDecisionTrace.finding via the same projectFinding path.
  test("inner finding.traceId is populated when the ledger row carries traceId", async () => {
    ddbMock.on(GetCommand).resolves({
      Item: ledgerRow({
        findingId: "f-78",
        decision: "permit",
    ...
    const result = (await handler(
      ...
      finding: { traceId: string | null };
    };

    expect(result.finding.traceId).toBe("1-5f2f0000-abcdef0123456789abcdef01");
  });

  test("inner finding.traceId is null when the ledger row predates stamping", async () => {
    ddbMock.on(GetCommand).resolves({
      Item: ledgerRow({
        findingId: "f-79",
        decision: "permit",
...
describe("listCaseLaw", () => {
  ...
  test("legacy float encodedAt is normalised to ISO-8601 string", async () => {
    ...
    expect(result).toHaveLength(1);
    expect(result[0].encodedAt).toBe(new Date(1715000000 * 1000).toISOString());
  });
...
describe("constitutionalRule mutations (Wave 4.C.2)", () => {
  ...
  test.each([
    ...
    [
      "deleteConstitutionalRule",
      { layerId: "l1", ruleIndex: 0, acknowledgement: ACK_TEXT },
    ],
  ] as const)(
    "%s: admin gate throws Forbidden for non-admin",
    async (fieldName, input) => {
      isAdminFromEventMock.mockReturnValue(false);
      await expect(handler(adminMutEvent(fieldName, input))).rejects.toThrow(
  ...
  test.each([
    ...
    [
      "deleteConstitutionalRule",
      { layerId: "l1", ruleIndex: 0, acknowledgement: "wrong" },
    ],
  ] as const)(
    "%s: acknowledgement string mismatch throws",
    async (fieldName, input) => {
      await expect(handler(adminMutEvent(fieldName, input))).rejects.toThrow(
        "Acknowledgement string mismatch",
      );
    },
  );
  ...
  test.each([
    [
      ...
      {
        layerId: "l1",
        rule: { field: "f", operator: "bogus", value: '"x"' },
        acknowledgement: ACK_TEXT,
      },
    ],
    [
      "updateConstitutionalRule",
      {
        layerId: "l1",
        ruleIndex: 0,
        newRule: { field: "f", operator: "bogus", value: '"x"' },
        acknowledgement: ACK_TEXT,
      },
    ],
  ] as const)(
    ...
    async (fieldName, input) => {
      ddbMock
        .on(GetCommand)
        .resolves({ Item: makeStoredLayerRow({ layerId: "l1" }) });
      await expect(handler(adminMutEvent(fieldName, input))).rejects.toThrow(
...
describe("caseLaw mutations (Wave 4.D.2)", () => {
  ...
  function adminCaseLawEvent(
    fieldName: "revokeCaseLaw" | "unrevokeCaseLaw" | "updateCaseLawPrecedence",
    input: Record<string, unknown>,
  ): HandlerEvent {
    return makeEvent({
      fieldName,
  ...
  test.each([
    ...
    [
      "unrevokeCaseLaw",
      { caseId: "case-1", acknowledgement: CASELAW_ACK_TEXT },
    ],
    [
  ...
  ] as const)("%s: empty caseId throws", async (fieldName, input) => {
    await expect(handler(adminCaseLawEvent(fieldName, input))).rejects.toThrow(
  ...
  test("revoke: sets revoked=true; emits audit event with previousValue/newValue", async () => {
    ...
    expect(result.emittedEventDetailType).toBe("governance.caselaw.changed");
...
describe("authorityGraphHistorySettings (Wave 4.E.A)", () => {
  ...
  const SETTINGS_PARAM_PREFIX = "/citadel/governance/authority-graph-history/";
  ...
  test("getAuthorityGraphHistorySettings: returns parsed value when SSM present", async () => {
    stubSettingsSsm({
      storedValue: '{"enabled":true,"retentionDays":90,"captureMode":"daily"}',
    });
    const result = (await handler(
      settingsEvent("getAuthorityGraphHistorySettings"),
    )) as {
  ...
  test("getAuthorityGraphHistorySettings: lastSnapshotAt = max timestamp from scan", async () => {
    stubSettingsSsm({
      storedValue: '{"enabled":true,"retentionDays":7,"captureMode":"daily"}',
    });
  ...
  test("getAuthorityGraphHistorySettings: scan filter expression bounds the retention window", async () => {
    stubSettingsSsm({
      storedValue: '{"enabled":true,"retentionDays":30,"captureMode":"daily"}',
    });
    ddbMock.on(ScanCommand).resolves({ Items: [] });
    ...
    expect(scanInput.TableName).toBe("citadel-governance-graph-snapshots-test");
  ...
  test('updateAuthorityGraphHistorySettings: captureMode "on-change" accepted', async () => {
    stubSettingsSsm({
      storedValue: '{"enabled":false,"retentionDays":30,"captureMode":"daily"}',
    });
    const result = (await handler(
      settingsEvent("updateAuthorityGraphHistorySettings", {
        input: {
...
describe("listAuthorityGraphSnapshots (Wave 4.E.B)", () => {
  ...
  test("365-day window cap clamps an over-wide window", async () => {
    ...
    const input = ddbMock.commandCalls(QueryCommand)[0].args[0].input;
    const effectiveSince = input.ExpressionAttributeValues?.[
      ":since"
    ] as number;
    const effectiveUntil = input.ExpressionAttributeValues?.[
      ":until"
    ] as number;
...
describe("getD4RetrospectiveReport", () => {
  ...
  test("truncation flag set when scanned items exceed 5000", async () => {
    ...
    for (let i = 0; i < 5500; i++) {
      items.push(
        makeD4Row({
          ...
          reason: `r-${i}`,
          scope_evaluated:
            i % 2 === 0 ? "worker-pre-filter" : "worker-tool-handler",
        }),
      );
    }
    ddbMock.on(ScanCommand).resolves({ Items: items });
    ...
    expect(result.counts.preFilterTotal + result.counts.toolHandlerTotal).toBe(
      5000,
    );
    expect(
      ...
      Statement: [
        {
          Effect: "Allow",
          Principal: {
            AWS: ["arn:aws:iam::123:role/r1", "arn:aws:iam::123:role/r2"],
          },
          Action: "sts:AssumeRole",
        },
      ],
...
describe("getTrustPath", () => {
  ...
  test("datastore without crossAccountRoleArn → hops = [lambda, scoped]", async () => {
    ...
    iamMock
      .on(GetRoleCommand)
      .callsFake(async (input: GetRoleCommandInput) => ({
        Role: {
    ...
    iamMock
      ...
      .callsFake(async (input: GetRolePolicyCommandInput) => ({
        RoleName: input.RoleName,
        PolicyName: "DataStoreAccess",
  ...
  test('GetRole missing → hop has empty principals + note "Role <arn> not found"', async () => {
    ...
    expect(
      result.notes.some(
        (n) => n.includes("not found") && n.includes(scopedHop!.arn),
      ),
    ).toBe(true);
  });
  ...
  test("GetRolePolicy missing → hop has empty inlinePolicy + note", async () => {
    ...
    expect(
      result.notes.some(
        (n) =>
          n.includes("Inline policy not present") && n.includes(scopedHop!.arn),
      ),
    ).toBe(true);
  });
...
describe("getResourceIamDrift", () => {
  ...
  beforeEach(() => {
    requiredPoliciesMock = jest.fn().mockReturnValue({
      connect: [{ actions: ["s3:GetObject"], resources: ["arn:aws:s3:::b/*"] }],
    });
  ...
  test("returns degraded report when resource not found", async () => {
    ...
    expect(result.notes.some((n) => n.includes("Resource not found:"))).toBe(
      true,
    );
  });
  ...
  test("returns degraded report when adapter not registered", async () => {
    ...
    expect(result.groups).toEqual([]);
    expect(
      result.notes.some((n) => n.includes("Adapter not registered for type")),
    ).toBe(true);
  });
