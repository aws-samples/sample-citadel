...
describe("backfill-project-org", () => {
  ...
  test("updates every item the Scan page returns to Default (server-side FilterExpression already excludes scoped rows)", async () => {
    ...
    const updatedIdsFromCalls = updateCalls.map(
      (c) => (c.args[0].input.Key as { id: string }).id,
    );

    expect(result.scanned).toBe(2);
    expect(result.updatedIds.sort()).toEqual([
      "proj-missing-org",
      "proj-null-org",
    ]);
    expect(updatedIdsFromCalls.sort()).toEqual([
      "proj-missing-org",
      "proj-null-org",
    ]);
