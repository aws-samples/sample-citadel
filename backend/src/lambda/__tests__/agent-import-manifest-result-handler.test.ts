...
const proposedEvent = (detailOverrides: Record<string, unknown> = {}) =>
  ({
    ...
    detail: {
      ...
      status: "proposed",
      // Originating caller's server-derived org (finding ce470ab0) — must
      // match BASE_META.orgId for the park to proceed. Individual tests
      // override this to exercise the mismatch/missing-org paths.
      orgId: "org-1",
      ...detailOverrides,
    },
...
describe("agent-import-manifest-result-handler", () => {
  ...
  describe("proposed", () => {
    ...
    it("NEVER mutates manifest / invocation / state / governanceAttestation", async () => {
      ...
      expect(written.governanceAttestation).toEqual(
        BASE_META.governanceAttestation,
      );
  ...
  // ─── Org reconciliation (finding ce470ab0, item 5) ───────────────────

  describe("org reconciliation between the Fabricator envelope and the target record", () => {
    it("mismatched-org envelope performs no write (proposed)", async () => {
      const errSpy = jest.spyOn(console, "error").mockImplementation(() => {});

      await handler(proposedEvent({ orgId: "org-attacker" }));

      expect(mockUpdateResource).not.toHaveBeenCalled();
      const loggedMismatch = errSpy.mock.calls.some((args) =>
        args.some(
          (a) =>
            typeof a === "string" &&
            a.includes("orgId missing or does not match"),
        ),
      );
    ...
    it("mismatched-org envelope performs no write (failed)", async () => {
      await handler(failedEvent({ orgId: "org-attacker" }));
      expect(mockUpdateResource).not.toHaveBeenCalled();
    });

    it("missing envelope orgId performs no write (treated as a mismatch, not a bypass)", async () => {
      await handler(proposedEvent({ orgId: undefined }));
      expect(mockUpdateResource).not.toHaveBeenCalled();
    });

    it("missing envelope orgId performs no write even when the record itself carries no orgId", async () => {
      mockGetResource.mockResolvedValue({
        recordId: "imp-1",
        name: "agent",
        status: "DRAFT",
    ...
    it("same-org envelope still writes normally (regression guard)", async () => {
      await handler(proposedEvent({ orgId: "org-1" }));
      expect(mockUpdateResource).toHaveBeenCalledTimes(1);
    });
  });
});
