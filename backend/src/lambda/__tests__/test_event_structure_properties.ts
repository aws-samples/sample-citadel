 ...
 *
 * For any valid app status transition (ACTIVE→PUBLISHED, PUBLISHED→DRAFT),
 ...
 * For ACTIVE→PUBLISHED, the event should additionally contain endpointUrl and apiKeyId.
 */
...
/** Detail fields for ACTIVE→PUBLISHED (includes endpointUrl and apiKeyId) */
...
const ACTIVE_TO_PUBLISHED: Transition = { from: 'ACTIVE', to: 'PUBLISHED' };
...
const transitionWithDetailArb = fc.oneof(
  publishDetailArb.map(detail => ({
    transition: ACTIVE_TO_PUBLISHED,
    detail,
  })),
...
describe('Property 3: Status transition event structure', () => {
   ...
   *
   * For ACTIVE→PUBLISHED transitions, the event detail additionally contains
   * endpointUrl and apiKeyId fields.
   */
  it('ACTIVE→PUBLISHED event additionally contains endpointUrl and apiKeyId', () => {
    fc.assert(
      fc.property(
        publishDetailArb,
        (detail) => {
