...
const AGENTCORE_INTEGRATION_TYPES = new Set([
  "AWS_LAMBDA",
  "AWS_SMITHY",
  "MCP_SERVER",
]);
...
function inferCredentialProviderType(
  integration:
    | { credentialProviderArn?: unknown; credentialProviderType?: unknown }
    | undefined,
): "API_KEY" | "OAUTH2" | undefined {
...
async function provisionProviderForIntegration(
  ...
  if (providerType === "OAUTH2") {
    ...
    if (provider.callbackUrl)
      result.agentCoreCallbackUrl = provider.callbackUrl;
    return result;
  }
  ...
  const provider = await provisionCredentialProvider(integrationId, "API_KEY", {
    apiKey,
  });
  return {
    credentialProviderArn: provider.credentialProviderArn,
...
async function emitIntegrationEvent(
  detailType:
    "integration.connect.requested" | "integration.disconnect.requested",
...
interface AppSyncEvent extends Omit<
  AppSyncResolverEvent<IntegrationResolverArguments>,
  "identity"
> {
 ...
 */
function sanitizeIntegrationForResponse(
...
async function createIntegration(
  ...
  const needsProvider = Boolean(
    getCredentialProviderType(input.integrationType, input.credentials),
  );
  const usesUnifiedAsyncFlow =
    isAgentCore || input.integrationType === "CONFLUENCE";
    ...
    if (needsProvider) {
      try {
        ...
        try {
          ...
          console.warn(
            "Failed to clean up secret after provision failure:",
            cleanupErr,
          );
        }
        throw new Error(
...
async function updateIntegration(
  ...
  if (input.credentials) {
    ...
    for (const configField of spec.configuration.required) {
      const configVal =
        input.config?.[configField] || integration.config[configField];
      if (configVal) {
    ...
    const existingData = existing.SecretString
      ? JSON.parse(existing.SecretString)
      : {};
  ...
  if (input.config) {
    ...
    for (const paramName of spec.configuration.ssmParameters) {
      const camelCaseKey = paramName.replace(/-([a-z])/g, (g) =>
        g[1].toUpperCase(),
      );
  ...
  if (input.status) {
    validateTransition(
      ...
      input.status as IntegrationStatus,
    );
...
async function testIntegration(integrationId: string, event: unknown) {
    ...
    if (spec && spec.configuration.ssmParameters) {
      for (const paramName of spec.configuration.ssmParameters) {
        try {
          ...
          const configKey = paramName.replace(/-([a-z])/g, (g) =>
            g[1].toUpperCase(),
          );
          config[configKey] = param.Parameter?.Value;
        } catch (error: unknown) {
          console.warn(
            `Failed to retrieve SSM parameter ${paramName}:`,
  ...
  try {
    ...
    if (testResult.success) {
      ...
      const newStatus = getStatusAfterSuccessfulTest(
        integration.status as IntegrationStatus,
      );
      ...
      const newStatus = getStatusAfterFailedTest(
        integration.status as IntegrationStatus,
      );
      integration.status = newStatus;
    }
