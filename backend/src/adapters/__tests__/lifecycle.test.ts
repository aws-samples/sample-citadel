...
describe("LifecycleManager", () => {
  describe("Integration lifecycle", () => {
    ...
    test("CONNECTING -> CONNECTION_FAILED is valid", () => {
      expect(
        lifecycle.isValidTransition("CONNECTING", "CONNECTION_FAILED"),
      ).toBe(true);
    });

    test("CONNECTED -> DISCONNECTED is valid", () => {
      expect(lifecycle.isValidTransition("CONNECTED", "DISCONNECTED")).toBe(
        true,
      );
    });
    ...
    test("same status is always valid (idempotent)", () => {
      expect(lifecycle.isValidTransition("CONFIGURED", "CONFIGURED")).toBe(
        true,
      );
    });
...
describe("validateTransitionWithHook", () => {
  ...
  test("valid transition + hook resolves -> resolves", async () => {
    ...
    await expect(
      lifecycle.validateTransitionWithHook(
        "CREATED",
        "IN_PROGRESS",
        hook,
        context,
...
describe("REGISTRY_TRANSITIONS", () => {
  ...
  describe("invalid transitions", () => {
    ...
    test.each(invalidCases)("%s → %s throws", (from, to) => {
      expect(() => lifecycle.validateTransition(from, to)).toThrow(
