...
/**
 * IMPLEMENT-list: connector types whose `testConnection` is implemented in
 * `utils/connection-tester`. These MUST be wrapped in
 * `ConnectionTesterIntegrationAdapter` so the unified-registry path is
 * structurally equivalent to the integration-resolver path. Wrapping any of
 * these in the bare `BaseIntegrationAdapter` would silently regress the
 * registry path (the bare base throws `NotImplementedError` on
 * `testConnection`, which is the loud-failure backstop for un-wired types).
 *
 * To add a new connector here, add a real test branch in
 * `utils/connection-tester.ts` first, then add its key to this set.
 */
const CONNECTION_TESTER_BACKED_TYPES = new Set<string>([
  'CONFLUENCE',
  'SERVICENOW',
  'JIRA',
  'SLACK',
...
function buildIntegrationAdapters(): Record<string, ConnectorAdapter> {
  ...
  for (const [key, spec] of Object.entries(BACKEND_CONNECTOR_REGISTRY)) {
    adapters[key] = CONNECTION_TESTER_BACKED_TYPES.has(key)
      ? new ConnectionTesterIntegrationAdapter(spec)
      : new BaseIntegrationAdapter(spec);
  }
  return adapters;
}
