...
describe("PolicyManager (promoted to utils)", () => {
  ...
  test("getRoleName works for all scopes", () => {
    expect(PolicyManager.getRoleName("abc", "datastore")).toBe(
      "citadel-ds-abc",
    );
    expect(PolicyManager.getRoleName("abc", "integration")).toBe(
      "citadel-int-abc",
    );
    expect(PolicyManager.getRoleName("abc", "agent")).toBe("citadel-agent-abc");
    // CIT-102: per-eval-run scoped role, prefix citadel-eval-.
    expect(PolicyManager.getRoleName("run-abc", "eval")).toBe(
      "citadel-eval-run-abc",
    );
  });

  test("ensureRole creates role with eval scope (CIT-102)", async () => {
    ...
    expect(createCalls[0].args[0].input.RoleName).toBe("citadel-eval-run-abc");
  });
...
describe("computeIntegrationPolicies", () => {
  ...
  test("returns secret-read policy scoped to the integration secret ARN", () => {
    const policies = computeIntegrationPolicies(
      ...
      {
        secretArn:
          "arn:aws:secretsmanager:us-west-2:123456789012:secret:/citadel/integrations/org1/confluence-int-abc",
      },
      accountId,
      region,
    );
    ...
    const secretPolicy = policies.find((p) =>
  ...
  test("returns lambda:InvokeFunction for AWS_LAMBDA type with lambdaArn", () => {
    const policies = computeIntegrationPolicies(
      ...
      "AWS_LAMBDA",
      {
        secretArn: "arn:aws:secretsmanager:us-west-2:123456789012:secret:test",
        config: {
          lambdaArn: "arn:aws:lambda:us-west-2:123456789012:function:MyFunc",
        },
      },
      accountId,
      region,
    ...
    const lambdaPolicy = policies.find((p) =>
  ...
  test("returns SSM read policy scoped to the integration prefix", () => {
    ...
    const ssmPolicy = policies.find((p) =>
      p.actions.includes("ssm:GetParameter"),
    );
...
describe("computeAgentPolicies", () => {
  ...
  test("returns bedrock:InvokeModel scoped to declared models", () => {
    ...
    const bedrockPolicy = policies.find((p) =>
      p.actions.includes("bedrock:InvokeModel"),
    );
    expect(bedrockPolicy).toBeDefined();
    expect(bedrockPolicy!.resources).toHaveLength(1);
    expect(bedrockPolicy!.resources[0]).toContain(
  ...
  test("returns datastore access policies when dataStores declared", () => {
    ...
    const stsPolicy = policies.find((p) =>
      p.actions.includes("sts:AssumeRole"),
    );
  ...
  test("combines model, datastore, and integration policies", () => {
    ...
    expect(
      policies.find((p) => p.actions.includes("bedrock:InvokeModel")),
    ).toBeDefined();
    expect(
      policies.filter((p) => p.actions.includes("sts:AssumeRole")),
    ).toHaveLength(2);
  });
});
