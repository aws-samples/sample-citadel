...
describe('emitMetrics — EMF envelope', () => {
  ...
  test('honours a custom unit', () => {
    ...
    expect(aws.CloudWatchMetrics[0].Metrics).toEqual([{ Name: 'ToolCalls', Unit: 'Count' }]);
  });
  ...
  test('drops non-finite metric values from directive and top level', () => {
    ...
    const blob = emitted();
    const aws = blob._aws as EmfEnvelope;
    expect(aws.CloudWatchMetrics[0].Metrics).toEqual([{ Name: 'Good_ms', Unit: 'Milliseconds' }]);
  ...
  test('never throws on completely invalid input', () => {
    expect(() => emitMetrics(undefined as unknown as Parameters<typeof emitMetrics>[0])).not.toThrow();
    expect(() => emitMetrics(null as unknown as Parameters<typeof emitMetrics>[0])).not.toThrow();
    expect(() =>
  ...
  test('never throws on a circular properties object and still emits the metric', () => {
    ...
    expect(logSpy).toHaveBeenCalledTimes(1);
    const blob = JSON.parse(logSpy.mock.calls[0][0] as string) as Record<string, unknown>;
