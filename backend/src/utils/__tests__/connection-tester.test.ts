...
describe('Connection Tester - Unit Tests', () => {
  ...
  describe('testConnection', () => {
    ...
    test('should route to testOAuth2Connection for Slack', async () => {
      ...
      // testOAuth2Connection now throws NotImplementedError on the
      // success path; testConnection's try/catch converts that to a
      // failure TestResult.
  ...
  describe('testOAuth2Connection', () => {
    test('should throw NotImplementedError for valid Slack OAuth credentials', async () => {
      const credentials = {
        clientId: 'test-client-id',
        clientSecret: 'test-client-secret',
        workspaceId: 'T01234ABCDE'
      ...
      await expect(
        testOAuth2Connection('SLACK', credentials, spec)
      ).rejects.toThrow(NotImplementedError);
      await expect(
        testOAuth2Connection('SLACK', credentials, spec)
      ).rejects.toThrow(/SLACK OAuth2\.testOAuth2Connection is not implemented/);
    });

    test('should throw NotImplementedError for valid Microsoft OAuth credentials', async () => {
      const credentials = {
        clientId: 'test-client-id',
        clientSecret: 'test-client-secret',
      ...
      await expect(
        testOAuth2Connection('MICROSOFT', credentials, spec)
      ).rejects.toThrow(NotImplementedError);
      await expect(
        testOAuth2Connection('MICROSOFT', credentials, spec)
      ).rejects.toThrow(/MICROSOFT OAuth2\.testOAuth2Connection is not implemented/);
    });
