...
describe("agent-import descriptor extension (US-IMP-001)", () => {
  ...
  describe("serialize → deserialize round-trip identity (property, 500 runs)", () => {
    ...
    const protocolArb: fc.Arbitrary<AgentInvocationProtocol> = fc.constantFrom(
      ...PROTOCOLS,
    );
    const authModeArb: fc.Arbitrary<AgentInvocationAuthMode> = fc.constantFrom(
      ...AUTH_MODES,
    );
    const modeArb: fc.Arbitrary<AgentInvocationMode> = fc.constantFrom(
      ...MODES,
    );
