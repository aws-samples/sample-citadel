...
describe('RegistryService record mapping', () => {
  ...
  describe('mapToAgentConfig', () => {
    ...
    it('maps PENDING_APPROVAL status to pending state', () => {
      const record: RegistryRecord = {
        recordId: 'a1',
        name: 'A',
        status: RegistryRecordStatusValues.PENDING_APPROVAL,
      };
      expect(service.mapToAgentConfig(record).state).toBe('pending');
    });
  ...
  describe('mapToToolConfig', () => {
    ...
    it('maps status to state correctly', () => {
      const cases: Array<[string, string]> = [
        ...
        [RegistryRecordStatusValues.PENDING_APPROVAL, 'pending'],
      ];
