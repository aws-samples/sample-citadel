...
export function computeCounts(
  ...
  return {
    ...
    connectedIntegrations: integrations.filter(i => i.status?.toLowerCase() === 'connected').length,
    ...
    deltas: {
      ...
      integrations: integrations.filter(i => i.status?.toLowerCase() === 'connected' && isRecent(i.createdAt)).length,
    },
  };
}
...
export function useDashboardData(): DashboardData {
  ...
  const fetchData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      const p = results[0].status === 'fulfilled' ? (results[0].value || []) : projects;
