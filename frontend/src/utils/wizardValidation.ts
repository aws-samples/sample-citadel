 ...
 *  2 = Workflows (no required fields — optional step; apps may bind agents
 *                  only and skip workflows entirely)
...
export function validateWizardStep(step: number, data: WizardStepData): ValidationResult {
  switch (step) {
    ...
    case 2:
      // Workflows are optional — apps may expose agents directly without a
      // multi-step workflow. Empty selection is valid.
