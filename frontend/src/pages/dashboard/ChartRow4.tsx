...
export default function ChartRow4({ counts, dataStores }: ChartRow4Props) {
        ...
        <Card className="w-1/2 bg-accent">
          ...
          <CardContent>
            <div className="flex flex-col gap-4">
              ...
              <button className="w-full mt-4 py-2 text-center text-foreground text-sm font-medium hover:bg-accent rounded-lg transition-colors border border-border">
                View All Activity
              </button>
            </div>
          </CardContent>
  ...
  return (
    <div>
      ...
      <div className="w-full rounded-full bg-border-border h-2">
        <div className="rounded-full bg-white h-2" style={{ width: `${value}%` }} />
      </div>
    </div>
  );
}
...
function QuickActionButton({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <button className="inline-flex items-center gap-2 px-4 py-2 bg-accent border border-border text-foreground text-sm font-medium rounded-lg hover:bg-accent transition-colors">
      {icon}{label}
    </button>
  );
}
