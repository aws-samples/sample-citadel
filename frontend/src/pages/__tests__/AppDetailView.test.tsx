...
describe('AppDetailView', () => {
  ...
  // Req 11.9: Status transition actions — Publish button for DRAFT
  it('shows Publish button when app is in DRAFT status', async () => {
    render(<AppDetailView {...defaultProps} />);
  ...
  // Req 11.9: Status transition actions — Archive button for ACTIVE
  it('shows Archive button when app is in ACTIVE status', async () => {
    (appApiService.getApp as jest.Mock).mockResolvedValue({ ...mockApp, status: 'ACTIVE' });
  ...
  // Req 11.9: Status transition actions — Reactivate button for ARCHIVED
  it('shows Reactivate button when app is in ARCHIVED status', async () => {
    (appApiService.getApp as jest.Mock).mockResolvedValue({ ...mockApp, status: 'ARCHIVED' });
