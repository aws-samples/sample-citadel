...
const activeApp = { ...baseApp, status: 'APPROVED' as const };
...
describe('AppDetailView — Publish Flow', () => {
  ...
  it('does not show "Publish" button when app status is DRAFT', async () => {
    ...
    // DRAFT shows the "Activate" transition button, not "Publish".
    // The ACTIVE/APPROVED → PUBLISHED "Publish" button must not be present.
    expect(screen.queryAllByText('Publish')).toHaveLength(0);
  });
  ...
  it('shows PUBLISHED status badge with purple styling', async () => {
    ...
    expect(publishedBadge?.className).toContain('bg-chart-5/20');
    expect(publishedBadge?.className).toContain('text-chart-5');
  });
