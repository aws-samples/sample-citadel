        ...
        <div className="absolute right-0 top-full mt-1 bg-accent border border-input rounded-md min-w-[200px] z-50 p-1 shadow-lg">
          {items.map(({ mode, icon: Icon, label }) => (
            <button
              ...
              className="flex items-center gap-2 w-full px-3 py-2 border-none bg-transparent text-muted-foreground text-[13px] cursor-pointer rounded text-left hover:bg-border-border transition-colors"
            >
...
export function Tools() {
  ...
  const { queueItems, error: queueError, isLoading: queueLoading, reload: reloadQueue, addPendingItem } = useFabricatorQueue({
  ...
  const handleConfigureTool = (toolId: string) => {
    alert(`Configure tool: ${toolId} - Coming soon!`);
  };
