...
export function Integrations() {
  ...
  // Post-create callback URL surface (shown once, after a successful OAuth2
  // MCP_SERVER createIntegration that returns an AgentCore Identity callback).
  const [callbackUrlInfo, setCallbackUrlInfo] = useState<{
    url: string;
  ...
  const handleAddIntegration = async (formData: ConnectorFormData) => {
    ...
    try {
      ...
      // For OAuth2 MCP_SERVER integrations, AgentCore Identity returns a callback URL
      // that the user must register at their OAuth provider as a redirect URI.
      // Display it once, immediately after creation (not on subsequent visits).
      if (result?.agentCoreCallbackUrl) {
        setCallbackUrlInfo({
          url: result.agentCoreCallbackUrl,
      ...
      console.error('Error object: [redacted]');
      console.error('Error message:', error.message);
      console.error('Error stack: [redacted]');
      console.error('Error details: [redacted]');
  ...
  const handleTest = async (integration: Integration) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error('Test failed: [redacted]');
        setMessage({ type: 'error', text: result.message || 'Connection test failed' });
      }
    } catch (error: any) {
      console.error('Test error (exception): [redacted]');
      console.error('Error details: [redacted]');
  ...
  const handleConnect = async (integration: Integration) => {
    ...
    try {
      ...
      const connectResult = await integrationServiceBackend.connectIntegration(integration.integrationId);
      console.log('connectIntegration returned', connectResult);

      // 3LO Authorization Code redirect: the backend returns an authorizationUrl
      // when the integration's grant type requires user consent. Use full-page
      // navigation (not popup) so SameSite cookies on the IdP work correctly.
      // We funnel through `navigateExternal` (a `window.location.assign` wrapper)
      // so unit tests can mock the navigation; jsdom does not allow overriding
      // `Location.prototype.assign` directly.
      if (connectResult?.authorizationUrl) {
        setMessage({ type: 'success', text: 'Redirecting to authorization provider...' });
        navigateExternal(connectResult.authorizationUrl);
        return;
      }
  ...
  return (
    <PageContainer className="flex flex-col w-full max-w-full box-border min-w-0">
      ...
      {/* Post-create AgentCore Callback URL Dialog
          Shown once after a successful OAuth2 MCP_SERVER createIntegration that
          returns an `agentCoreCallbackUrl`. The user must register this URL with
          their OAuth provider as a redirect URI before clicking Connect. */}
      <Dialog
        open={!!callbackUrlInfo}
        onOpenChange={(open: boolean) => {
          if (!open) {
            setCallbackUrlInfo(null);
            setCallbackCopied(false);
          }
        }}
      >
        <DialogContent
          className="bg-card border border-border sm:max-w-2xl w-[90vw]"
          data-testid="callback-url-dialog"
        >
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-foreground">
              <Key className="size-5" />
              Register this callback URL at your OAuth provider:
            </DialogTitle>
            <DialogDescription className="text-muted-foreground">
              {callbackUrlInfo?.integrationName
                ? `Add this redirect URI to the OAuth client configured for "${callbackUrlInfo.integrationName}". Without it, the Authorization Code (3LO) flow will fail.`
                : 'Add this redirect URI to your OAuth client configuration.'}
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-4 py-4">
            <div
              className="flex items-center gap-2 p-3 rounded-md bg-muted/50 border border-border font-mono text-sm break-all select-all"
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={async () => {
                  if (!callbackUrlInfo?.url) return;
                  try {
                    if (navigator?.clipboard?.writeText) {
                      await navigator.clipboard.writeText(callbackUrlInfo.url);
                    }
              ...
              >
                ...
                {callbackCopied ? 'Copied' : 'Copy to clipboard'}
              </Button>
              <Button
                type="button"
                size="sm"
                className="ml-auto"
                data-testid="continue-callback-url"
                onClick={() => {
                  setCallbackUrlInfo(null);
                  setCallbackCopied(false);
                }}
