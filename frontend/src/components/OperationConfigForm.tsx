        ...
        <div key={param.name} className="flex flex-col gap-1">
          ...
          {param.type === 'boolean' ? (
            <button
              ...
              type="button"
              role="switch"
              aria-checked={!!values[param.name]}
              onClick={() => handleFieldChange(param.name, !values[param.name])}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                values[param.name] ? 'bg-primary' : 'bg-accent'
              }`}
            >
              <span
                className={`inline-block size-4 transform rounded-full bg-white transition-transform ${
                  values[param.name] ? 'translate-x-6' : 'translate-x-1'
                }`}
              />
            </button>
          ) : param.type === 'number' ? (
