...
function findPhaseContainer(container: HTMLElement, phaseName: string): HTMLElement {
  ...
  let el: HTMLElement | null = heading;
  while (el && el !== container) {
    // StatusCard containers share the stable signature `rounded-lg p-4`
    // (see ProjectCard.tsx:105/120/137); the previous `w-1/3` matcher was made
    // stale by the migration to `grid grid-cols-1 sm:grid-cols-3`.
    if (el.className && el.className.includes('rounded-lg') && el.className.includes('p-4')) {
      return el;
    }
    el = el.parentElement;
  }
