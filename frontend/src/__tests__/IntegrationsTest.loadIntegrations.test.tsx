...
describe('Property 1: Fault Condition – Hardcoded Mocks Displayed Instead of Backend Integrations', () => {
  ...
  /**
   * Scoped PBT case 3:
   * Assert `listIntegrations` is called with the orgId. Status filtering for
   * "CONNECTED" happens client-side, not as an API parameter, so the mock
   * should only see one argument.
   *
   * **Validates: Requirements 1.3**
   */
  it('should call listIntegrations with orgId (status filter applied client-side)', async () => {
    mockListIntegrations.mockResolvedValue([]);
