...
callback_types = st.sampled_from(["eventbridge", "sqs", "mcp", "unknown_type"])
...
class TestCreateWorkflowTrackingRecord:
    ...
    def test_all_nodes_start_false(self, nodes):
        ...
        item = call_args[1].get("Item") or call_args[0][0] if call_args[0] else call_args[1]["Item"]
...
class TestSendResponse:
    ...
    @given(
        ...
        cb_type=st.text(min_size=1, max_size=20).filter(
            lambda s: s not in ("eventbridge", "sqs", "mcp")
        ),
    )
    @settings(max_examples=30)
