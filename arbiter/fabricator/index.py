...
def _get_agentcore_client():
    """Create a thin wrapper that calls bedrock-agentcore-control via signed HTTP.

    The Lambda runtime's boto3/botocore is too old to know this service.
    Instead of fighting the import system, we make raw SigV4-signed requests.
    """
    return _AgentCoreRegistryClient()


class _AgentCoreRegistryClient:
    """Minimal client for bedrock-agentcore-control Registry APIs using SigV4."""
    ...
    def __init__(self):
        ...
        self._endpoint = f"https://bedrock-agentcore-control.{self._region}.amazonaws.com"

    def _sign_and_send(self, method, path, body=None):
        import botocore.auth
        ...
        request = botocore.awsrequest.AWSRequest(method=method, url=url, data=data, headers=headers)
        credentials = self._session.get_credentials().get_frozen_credentials()
        signer = botocore.auth.SigV4Auth(credentials, 'bedrock-agentcore', self._region)
        ...
        try:
            ...
            print(f"[AgentCoreClient] HTTP {e.code}: {error_body}")
    ...
    def create_registry_record(self, **kwargs):
        ...
        body = {k: v for k, v in kwargs.items() if k != 'registryId'}
        return self._sign_and_send('POST', f'/registries/{registry_id}/records', body)

    def update_registry_record_status(self, **kwargs):
        registry_id = kwargs['registryId']
        record_id = kwargs['recordId']
        body = {k: v for k, v in kwargs.items() if k not in ('registryId', 'recordId')}
        return self._sign_and_send('PATCH', f'/registries/{registry_id}/records/{record_id}/status', body)
...
def store_agent_config_registry(
    ...
    Custom metadata (categories, icon, state, manifest, appId) is serialized as
    JSON and stored in the CUSTOM descriptor's inlineContent. After creation the
    record status is moved to DRAFT so it requires activation before use
    (DRAFT maps to internal state "inactive" for fabricator-created records,
    per Requirement 8.3).
    ...
    try:
        ...
        # resourceDescription carries the runtime execution config. Stored as
        # a JSON string so the cache sync / resolvers can parse it verbatim.
        config = {
            "name": agent_id,
            "filename": file_name.split('/')[-1],
            "schema": llm_tool_schema,
        ...
        # set to DRAFT below via UpdateRegistryRecordStatus.
        custom_metadata = {
            "categories": ["worker"],
            "icon": "",
            "state": "inactive",
        ...
        response = client.create_registry_record(
            ...
            name=agent_id,
            description=json.dumps(config, default=str),
...
def store_tool_config_registry(
    ...
    Custom metadata (categories, icon, state, integrationBindings,
    dataStoreBindings, appId) is serialized as JSON and stored in the CUSTOM
    descriptor's inlineContent. After creation the record status is moved to
    PUBLISHED so the tool is immediately usable (PUBLISHED maps to internal
    state "active" for fabricator-created tools, per Requirement 8.4).
    ...
    try:
        ...
        # resourceDescription carries the tool's execution config. Stored as
        # a JSON string so the cache sync / resolvers can parse it verbatim.
        config = {
            "name": tool_id,
            "filename": file_name.split('/')[-1],
        ...
        # set to PUBLISHED below via UpdateRegistryRecordStatus.
        custom_metadata = {
            "categories": [],
            "icon": "",
            "state": "active",
...
def create_tool_fabricator():
    ...
    tool_fabricator = Agent(
        bedrock_model,
        tools=[file_write, http_request, shell, upload_tool_to_s3, store_tool_config_registry],
...
def create_agent_fabricator(complete_task):
    ...
    agent_fabricator = Agent(
        ...
        tools=[file_write, http_request, shell, get_worker_tool, create_custom_tool,
            upload_agent_to_s3, store_agent_config_registry, complete_task],
