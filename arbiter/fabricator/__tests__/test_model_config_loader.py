"""Tests for the fabricator model-selection I/O loader.

Mirrors the supervisor loader test: the bulletproof fallback contract means
any missing env var, missing/empty config item, or exception must return the
caller-supplied fallback, while the happy path resolves the regional inference
profile from the config + catalog tables via the shared pure kernel. All ids
are generic placeholders, never real Bedrock ids.
"""
