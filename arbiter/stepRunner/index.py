...
def handler(event, context):
    ...
    # above only fires when the detail carries a traceContext
    # (carried-context hops); the plain EventBridge workflow-dispatch path
    # did not otherwise stamp run_id/execution_id/correlation_id, leaving
    # StepRunner spans unqueryable. execution_trace_scope keeps a single
    # annotated SUBSEGMENT open for the routed work below — annotating the
    # Lambda-owned FacadeSegment (current_segment()) drops the annotation
    # silently in Lambda, since FacadeSegment.put_annotation always raises.
